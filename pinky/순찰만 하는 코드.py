...
# ==== 여기 사이에 넣기 시작 ====
# ==== detour 설정 (1→2 구간에서 잠깐 나갔다가 복귀) ====
DETOUR_TRIGGER = (0.30, 0.41)   # 1->2 구간 중 트리거 지점
DETOUR_OUT     = (0.10, 0.41)   # 바깥으로 빠질 좌표
DETOUR_BACK    = (0.30, 0.41)   # 복귀 좌표(트리거와 동일)
DETOUR_RADIUS  = 0.12           # 트리거 판정 반경 (m) — 5cm
# ==== 여기까지 붙여넣기 ====
...
class ArucoNav(Node):
    def __init__(self):
        ...
        self.d_ang_filt = 0.0
        # detour 상태 (순찰에서만 사용)
        self.detour_stage = None             # None, 'OUT', 'BACK'
        self.detour_return_goal = None
        # === 미션 시작 구독 ===
        self.create_subscription(Int32, MISSION_TOPIC, self._mission_cb, 10)
        # 카메라 파라미터 로드
