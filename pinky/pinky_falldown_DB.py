...
FALL_IMAGE_DIR = '/home/a/fall_images/' # 이 변수는 이제 사용되지 않습니다.
# ====================================================================
...
class ArucoNav(Node):
    def __init__(self):
        ...
        self.fall_logged = False # 👈 추가: 쓰러짐 이벤트 로그 기록 여부 플래그
        ...
        # 이미지 저장 디렉토리 확인 및 생성
        if not os.path.exists(FALL_IMAGE_DIR):
            os.makedirs(FALL_IMAGE_DIR)
    ...
    # 쓰러짐 데이터 저장 메서드 (추가)
    def save_fall_data(self):
        """쓰러짐 데이터를 DB에 저장하고 이미지 파일을 캡처합니다."""
        if not self.db_conn:
        ...
        image_path = os.path.join(FALL_IMAGE_DIR, image_filename)
        
        # process_frame에서 저장된 마지막 프레임이 있는지 확인
        if hasattr(self, 'last_frame') and self.last_frame is not None:
            cv2.imwrite(image_path, self.last_frame)
            self.get_logger().info(f"쓰러짐 이미지 저장: {image_path}")
        else:
            self.get_logger().warn("이미지 캡처 실패: 유효한 프레임이 없습니다.")
            image_path = "Failed to capture image"
            
        # 4. DB에 데이터 삽입
        cursor = self.db_conn.cursor()
        sql = "INSERT INTO fall_down_logs (falldown_detected_time, falldown_image_path, falldown_location) VALUES (%s, %s, %s)"
        data = (now, image_path, location)
        
        try:
            cursor.execute(sql, data)
            self.db_conn.commit()
            self.get_logger().info("쓰러짐 데이터 DB 저장 성공 ✅")
            self.fall_logged = True # 성공 시 플래그 설정
        except mysql.connector.Error as err:
