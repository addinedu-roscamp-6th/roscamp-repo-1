...
# 픽업/드랍: 경로 위 앵커 + 경로 밖 실제 지점
PICKUP_ANCHOR  = (0.30, 0.41)
PICKUP_IN      = (0.10, 0.41)
DROPOFF_ANCHOR = (1.18, 0.95)
DROPOFF_IN     = (1.18, 0.68)
# === 픽업/드랍 실제 지점에서 대기 시간(초) ===
WAIT_AT = {
    PICKUP_IN: 3.0,     # 픽업 실제 지점에서 3초 정지
    DROPOFF_IN: 3.0,    # 드랍 실제 지점에서 3초 정지
}
SCALE_TO_M = 1.0
USE_CMD_VEL = True
...
class ArucoNav(Node):
    ...
    def _slice_cyclic(self, start_i, end_i):
        if start_i <= end_i:
            return self.patrol_waypoints[start_i:end_i+1]
        else:
            return self.patrol_waypoints[start_i:] + self.patrol_waypoints[:end_i+1]
    ...
    def check_and_advance_goal(self, x, y):
        if self.goal_idx >= len(self.waypoints):
            return
        # 이미 대기 중이면: 시간이 끝나면 즉시 다음 웨이포인트로 진행
        if self.waiting:
            now = self.get_clock().now()
            if now >= self.wait_end_time:
                # 대기 종료 → 다음 목표로 advance
                self.waiting = False
                self.wait_end_time = None
                ...
                if self.goal_idx >= len(self.waypoints):
                    if self.mode == 'MISSION':
                        self.get_logger().info(":흰색_확인_표시: 미션 완료. 순찰 복귀")
                        self.mode = 'PATROL'
        ...
        if d <= self.goal_radius:
            # 도착한 웨이포인트가 대기 대상이면 3초 대기 시작 (advance 하지 않음)
            wait_sec = WAIT_AT.get((gx, gy), 0.0)
            if wait_sec > 0.0:
                self.get_logger().info(f":일시_중지: 대기 시작 @ ({gx:.2f}, {gy:.2f}) for {wait_sec:.1f}s")
                now = self.get_clock().now()
                self.wait_end_time = now + Duration(seconds=float(wait_sec))
                self.waiting = True
                self.publish_goal(gx, gy)  # 목표 유지
                return
            # 평소처럼 다음 웨이포인트로
    ...
    def handle_keys(self):
        ...
        if key == ord('q'):
            ...
            if self.mode == 'PATROL':
                ...
                # 현재 순찰 인덱스, 앵커 인덱스
                cur_i  = self.goal_idx
                pick_i = self._closest_wp_index(PICKUP_ANCHOR)
                drop_i = self._closest_wp_index(DROPOFF_ANCHOR)
                # :흰색_확인_표시: 루프 위 최단(정/역) 경로 선택
                to_pick_pts = self._shortest_path_points(cur_i,  pick_i)      # 현재→픽업앵커
                pick_to_drop_pts = self._shortest_path_points(pick_i, drop_i) # 픽업앵커→드랍앵커
                # 첫/마지막 중복 제거 유틸
                def drop_first_if_same(seq, pt):
                    return seq[1:] if (len(seq) > 0 and seq[0] == pt) else seq
                # 미션 경로: 루프 유지 + 앵커 외출/복귀 + 3초 정지
                mission_path = []
                mission_path += to_pick_pts                                  # 루프 위 현재→픽업앵커
                mission_path += [PICKUP_IN, PICKUP_ANCHOR]                    # 앵커→실좌표→앵커 (픽업)
                mission_path += drop_first_if_same(pick_to_drop_pts, PICKUP_ANCHOR)  # 앵커 중복 제거
                mission_path += [DROPOFF_IN, DROPOFF_ANCHOR]                  # 앵커→실좌표→앵커 (드랍)
                self._set_waypoints(mission_path, reset_idx=True)
                self.get_logger().info(":트럭: 미션 시작: 루프 최단 방향으로 이동 + 픽업/드랍 앵커 외출/복귀 + 3초 정지")
            else:
                self.get_logger().info("이미 미션 진행 중이라 새 요청을 무시합니다.")
    # --------------------- 종료 ---------------------
