...
# 폴더 이름 설정
str_falldown = 'falldown_pose'
str_standing = 'standing_pose'

# CSV 파일 로드
df = pd.read_csv(keypoint_path)

# 쓰러진 이미지와 서있는 이미지 경로 설정
falldown_path = os.path.join(dataset_dir, str_falldown)
standing_path = os.path.join(dataset_dir, str_standing)

# 이미지 이름에 따라 레이블을 지정하는 함수 정의
def get_label(image_name, falldown_path, standing_path):
    if image_name in os.listdir(falldown_path):
        return 'falldown'
...
# CSV 파일에 'label' 열 추가
df['label'] = df['image_name'].apply(lambda x: get_label(x, falldown_path, standing_path))
